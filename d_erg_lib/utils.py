...
def convert_phi2phik(basis, phi_val, phi_grid=None):
    ...
    if phi_grid is None:
        ...
        phi_grid = np.meshgrid(*[np.linspace(0, 1., int(np.sqrt(len(phi_val)))) # i.e. num_pts from target_dist
                                for _ in range(2)])
        phi_grid = np.c_[phi_grid[0].ravel(), phi_grid[1].ravel()] # reconstructing grid from target_dist
    assert phi_grid.shape[0] == phi_val.shape[0], 'samples are not the same'

    return np.sum([basis.fk(x) * v for v, x in zip(phi_val, phi_grid)], axis=0) # dim = num_basis**2 aka k

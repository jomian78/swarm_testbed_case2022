...
class Visual(object):
    def __init__(self):
        ...
        self._marker_pub = rospy.Publisher('env/visual', MarkerArray, queue_size=1, latch=True)

        # waits for publisher to intiialize then deletes prior visualization before publishing new one
        while not self._marker_pub.get_num_connections():
            rospy.sleep(1.0)
        clear = MarkerArray([Marker(action=Marker.DELETEALL)])
        self._marker_pub.publish(clear)

        # builds new rendering
